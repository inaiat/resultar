...
`vite.config.ts` loads `resultar-lint/oxlint` and enables `resultar/no-discard`. The rule calls
Resultar's TypeScript-backed checker internally, so it reports ignored `Result` and `ResultAsync`
values plus assigned-but-unhandled Resultar values even though Oxlint custom JS rules do not expose
TypeScript services.
