...
const fetchUserRecord = async (id: string): Promise<User> => ({ email: `${id}@example.com`, id });
...
// yield-star-in-result-task-gen: ResultTask.gen composes lazy tasks with yield*.
export const loadUserWithResultTask = (id: string): ResultTask<User, FetchUserError> =>
...
// prefer-map: map is enough when the transformation cannot fail.
export const normalizedUserId = (email: string): StrictResult<string, InvalidEmailError> =>
  validateEmail(email).map((value) => `user:${value.trim().toLowerCase()}`);
