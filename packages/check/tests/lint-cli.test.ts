...
const runCli = (
  ...
  cwd: string,
  runner: (cliArgs: readonly string[]) => number = runResultarLintCli,
) => {
  ...
  try {
    return { status: runner(args), stderr, stdout };
  } finally {
...
`);
  ...
  it("handles additional CLI flag forms", async () => {
    ...
    const checkHelp = runCli(["--help"], cleanRoot, runResultarCheckCli);
    equal(checkHelp.status, 0);
    match(checkHelp.stdout, /Usage: resultar-check/);

    const checkMissingMode = runCli(["--mode"], cleanRoot, runResultarCheckCli);
    equal(checkMissingMode.status, 1);
    match(checkMissingMode.stderr, /--mode requires direct or must-use/);

    const checkEmptyProject = runCli(["--project="], cleanRoot, runResultarCheckCli);
    equal(checkEmptyProject.status, 1);
    match(checkEmptyProject.stderr, /--project requires a path/);
  });
