...
describe('Result.Ok', async () => {
  ...
  it('Replaces an Ok value with as', () => {
    const replaced = ok<number, string>(12).as('created')

    expectTypeOf(replaced).toEqualTypeOf<Result<string, string>>()
    isTrue(replaced.isOk())
    equal(replaced._unsafeUnwrap(), 'created')
  })
...
describe('Result.Err', async () => {
  ...
  it('Preserves an Err with as', () => {
    const replaced = err<number, string>('Wrong format').as('created')

    expectTypeOf(replaced).toEqualTypeOf<Result<string, string>>()
    isTrue(replaced.isErr())
    equal(replaced._unsafeUnwrapErr(), 'Wrong format')
  })
...
describe('ResultAsync', async () => {
  ...
  describe('as', async () => {
    it('Replaces an Ok value', async () => {
      const replaced = okAsync<number, string>(12).as('created')

      expectTypeOf(replaced).toEqualTypeOf<ResultAsync<string, string>>()

      const result = await replaced
      isTrue(result.isOk())
      equal(result._unsafeUnwrap(), 'created')
    })

    it('Preserves an Err', async () => {
      const replaced = errAsync<number, string>('Wrong format').as('created')

      expectTypeOf(replaced).toEqualTypeOf<ResultAsync<string, string>>()

      const result = await replaced
      isTrue(result.isErr())
      equal(result._unsafeUnwrapErr(), 'Wrong format')
    })

    it('Does not unwrap a Promise used as the replacement value', async () => {
      const replacement = Promise.resolve('created')
      const replaced = okAsync<number, string>(12).as(replacement)

      expectTypeOf(replaced).toEqualTypeOf<ResultAsync<Promise<string>, string>>()

      const result = await replaced
      isTrue(result.isOk())
      strictEqual(result._unsafeUnwrap(), replacement)
    })
  })
...
describe('Utils', async () => {
  describe('`Result.zip`', async () => {
    it('combines two successful results into a tuple', () => {
      const result: Result<[string, number], string | number> = Result.zip(ok('email'), ok(123))

      isTrue(result.isOk())
      deepEqual(result._unsafeUnwrap(), ['email', 123])
    })

    it('returns the first error for sync zipped results', () => {
      const result = Result.zip(err<string, string>('email error'), err<number, number>(123))

      isTrue(result.isErr())
      equal(result._unsafeUnwrapErr(), 'email error')
    })

    it('combines two successful async results into a tuple', async () => {
      const result: Result<[string, number], string | number> = await ResultAsync.zip(
        okAsync('email'),
        okAsync(123),
      )
    ...
    it('returns the first error for async zipped results', async () => {
      const result = await ResultAsync.zip(
        errAsync<string, string>('email error'),
        errAsync<number, number>(123),
      )
  ...
  describe('`Result.validateAll`', async () => {
    it('collects all sync validation errors from a result list', () => {
      const result = Result.validateAll([
        ok<number, string>(123),
        err<number, string>('boooom!'),
        ok<number, string>(456),
        err<number, string>('ahhhhh!'),
      ])
    ...
    it('maps all sync items before collecting validation errors', () => {
      const seen: number[] = []
      const result = Result.validateAll([1, 2, 3, 4], (value, index) => {
        seen.push(index)

        return value % 2 === 0 ? ok(value) : err(`${value} is odd`)
      })

      isTrue(result.isErr())
      deepEqual(result._unsafeUnwrapErr(), ['1 is odd', '3 is odd'])
      deepEqual(seen, [0, 1, 2, 3])
    })

    it('collects all sync values when every mapped item succeeds', () => {
      const result: Result<readonly number[], string[]> = Result.validateAll([1, 2, 3], (value) =>
        ok<number, string>(value * 2),
      )
    ...
    it('collects all async validation errors from a result list', async () => {
      const result = await ResultAsync.validateAll([
        okAsync<number, string>(123),
        errAsync<number, string>('boooom!'),
        okAsync<number, string>(456),
        errAsync<number, string>('ahhhhh!'),
      ])
    ...
    it('maps all async items before collecting validation errors', async () => {
      const seen: number[] = []
      const result = await ResultAsync.validateAll([1, 2, 3, 4], (value, index) => {
        seen.push(index)

        return value % 2 === 0 ? okAsync(value) : errAsync(`${value} is odd`)
      })
    ...
    it('collects all async values when every mapped item succeeds', async () => {
      const result: Result<readonly number[], string[]> = await ResultAsync.validateAll(
        [1, 2, 3],
        (value) => okAsync<number, string>(value * 2),
      )
...
describe('unwrapOrThrow', async () => {
  class CustomError extends Error {
    ...
    public constructor(message: string, customProperty: string) {
      super(message)
  ...
  describe('unwrapOrThrow sync', async () => {
    ...
    it('should throw error return a correct error', async () => {
      assert.throws(() => err('boooom!').unwrapOrThrow(), /^boooom!$/u)
      assert.throws(() => err(new Error('boooom!')).unwrapOrThrow(), /^Error: boooom!$/u)
  ...
  describe('unwrapOrThrow async', async () => {
    ...
    it('should throw error return a correct error', async () => {
      await assert.rejects(async () => await errAsync('boooom!').unwrapOrThrow(), /^boooom!$/u)
...
describe('tryResult', async () => {
  ...
  describe('Synchronous tryResult', async () => {
    ...
    it('should return err result with default error handling', () => {
      const result = tryResult(() => divide(10, 0))
      isTrue(result.isErr())
  ...
  describe('Asynchronous tryResult', async () => {
    it('should return ok result for successful async operation', async () => {
      const result = await tryResultAsync(divideAsync(10, 2))
      isTrue(result.isOk())
    ...
    it('should return err result for failed async operation', async () => {
      const result = await tryResultAsync(divideAsync(10, 0), (e) => (e as Error).message)
      isTrue(result.isErr())
  ...
  describe('Edge cases', async () => {
    it('should handle undefined and null operations correctly', () => {
      const undefinedResult = tryResult(() => undefined)
      isTrue(undefinedResult.isOk())
      equal(undefinedResult._unsafeUnwrap(), undefined)

      const nullResult = tryResult(() => null)
      isTrue(nullResult.isOk())
      equal(nullResult._unsafeUnwrap(), null)
    })

    it('should handle nested tryResult operations', async () => {
      const result = await tryResultAsync(
        (async () => {
          const inner = tryResult(() => divide(10, 0))
